...
function SWEP:Initialize()
	timer.Simple(0.1, function() 
		...
		if self:GetOwner():IsValid() and SERVER then
				self:GetOwner():SetHealth(GetConVar("gstands_thoth_heal"):GetInt())
				self:GetOwner():SetMaxHealth(GetConVar("gstands_thoth_heal"):GetInt())
			end
		end
	end)
end
...
hook.Add( "HUDShouldDraw", "ThothHud", function(elem)
	if GetConVar("gstands_draw_hud"):GetBool() and IsValid(LocalPlayer()) and IsValid(LocalPlayer():GetActiveWeapon()) and LocalPlayer():GetActiveWeapon():GetClass() == "gstands_thoth" and (((elem == "CHudWeaponSelection" ) and LocalPlayer().SPInZoom) or elem == "CHudHealth" or elem == "CHudAmmo" or elem == "CHudBattery" or elem == "CLHudSecondaryAmmo") then
		return false
	end
end)
