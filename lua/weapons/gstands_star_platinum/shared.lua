...
local StopTime = Sound( "weapons/platinum/time_stop_effect.wav" )
...
local Withdraugh = Sound( "weapons/platinum/o_sound.wav" )
...
if CLIENT then
	net.Receive( "platinum.PlaySound", function()
		if IsValid(LocalPlayer()) then
			LocalPlayer():EmitSound("weapons/platinum/time_stop-0"..math.random(1, 2)..".wav", 75, 100)
			timer.Simple(2, function()
				LocalPlayer():EmitSound(StopTime)
			end)
			LocalPlayer():ChatPrint( "『Star Platinum!』" ) 
...
function SWEP:Think()
	...
	if self.Succ then
		...
		for k,v in pairs(ents.FindInCone(self.Stand:GetEyePos(true), self.Stand:GetForward(), 350, 0.55)) do
	...
	if IsValid(self.Stand) and self.Stand:GetSequence() == self.Stand:LookupSequence( "starfinger" ) then
...
function SWEP:StartTime()
	if IsValid(self.Owner) then
		...
		--self.Owner:EmitSound(StartTimeVoice)
...
function SWEP:Barrage()
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0) ) then
		...
		dmginfo:SetDamage(GetConVar("gstands_star_platinum_barrage_damage"):GetInt())
	...
	--if 10 * self.Power >= tr.Entity:Health() then
...
function SWEP:DonutPunch()
	...
	if IsValid(self.Stand) then
		...
		local tr = util.TraceHull( {
			...
			endpos = self.Stand:GetBonePosition(self.Stand:LookupBone( "ValveBiped.Bip01_R_Hand" )) + self.Owner:GetAimVector() * self.HitDistance,
		...
		if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0) ) then
			...
			dmginfo:SetDamage(GetConVar("gstands_star_platinum_punch_damage"):GetInt())
...
function SWEP:Grab()
	...
	if IsValid(self.Stand) then
		...
		tr = util.TraceHull( {
			...
			endpos = self.Stand:WorldSpaceCenter() + self.Owner:GetAimVector() * 75,
...
function SWEP:StarFinger()
	...
	if IsValid(self.Stand) then
		...
		if IsValid(self.Stand) then
			...
			timer.Simple(self.Stand:SequenceDuration(), function() 
				if IsValid(self.Stand) and self:GetOwner():Alive() then
					self:ResetFingers() self.Hit = false 
				end
			end)
		end
...
function SWEP:Holster(wep)
	...
	if IsValid(self.Owner) then
		...
		self.Owner:StopSound("succ_loop")
