...
ENT.PrintName= "#gstands.names.lovers"
ENT.Author= "Copper"
ENT.Contact= ""
...
local Confirm = Sound( "lovers.confirm" )
...
function ENT:Initialize()
	...
	self.OwnerPos = self.Owner:GetPos()
	self:SetIdealPos(self.OwnerPos)
	self.Pos = self:GetPos()
...
function ENT:DoMovement()
	if !self:GetMarkedToFade() then
		...
		local offset = Vector()
		offset.z = math.cos(CurTime()) * 4 + math.sin(CurTime()) * 10
		offset.x = math.sin(CurTime()) * 4  + -math.cos(CurTime()) * 7
		offset.y = -math.cos(CurTime()) * 4  + math.sin(CurTime()) * 10
				self:SetIdealPos(self.Owner:GetPos() + self.Owner:GetUp() * 55 + offset)
				self.Pos = self:GetIdealPos()
		end
		...
		if IsValid(self.Wep:GetTarget()) then
			self:SetIdealPos(self.Wep:GetTarget():EyePos())
			self:SetNotSolid(true)
		end
		if not IsValid(self.Wep:GetTarget()) then
...
function ENT:Think()
	...
	if self.Owner:GetInfoNum("gstands_active_dododo_toggle", 1) == 1 and SERVER and CurTime() > self.DoDoDoTimer and self.Owner:gStandsKeyDown("dododo") and self.StandId != GSTANDS_JST then
		...
		if self:GetInDoDoDo() then
			self.Wep:SetTarget()
			self:SetIdealPos(self.Owner:EyePos())
			self:SetPos(self.Owner:EyePos())
			self.BrainTimer = CurTime() + 1
			local clr = gStands.GetStandColorTable(self.Model, self.Skin)
			clr.a = 150
			self:SetTrail(util.SpriteTrail(self, 4, clr, true, 0.6,0, 0.5, 1/(15) * 0.5, "effects/beam_generic01.vmt" ))
			elseif IsValid(self:GetTrail()) then
	...
	local braintrace = util.TraceLine({
			start = self.Pos - self.Owner:GetAimVector(),
			endpos = self.Pos + self.Owner:GetAimVector() * 75,
			filter = function(ent) if ent:IsPlayer() and ent != self.Owner then return true end end
		})
	self.BrainTimer = self.BrainTimer or CurTime()
	local pos = Vector(0,0,0)
	if braintrace.Hit then
	local eyepos = braintrace.Entity:EyePos()
	if braintrace.Entity:LookupBone("ValveBiped.Bip01_Head1") then
		eyepos = braintrace.Entity:GetBonePosition(braintrace.Entity:LookupBone("ValveBiped.Bip01_Head1"))
	end
	if CurTime() >= self.BrainTimer and braintrace.Hit and self:GetPos():Distance(eyepos) < 7 then
		local yes = true
		if braintrace.Entity.GetActiveWeapon and IsValid(braintrace.Entity:GetActiveWeapon()) and braintrace.Entity:GetActiveWeapon().GetStand and IsValid(braintrace.Entity:GetActiveWeapon():GetStand()) and braintrace.Entity:GetActiveWeapon():GetStand():GetClass() == "stand" and braintrace.Entity:GetActiveWeapon():GetStand():GetBlockCap() then
			yes = false
			self.StandVelocity = -self:GetForward() * 3
			self:SetPos(self:GetPos() - self:GetForward() * 55)
		end
		if yes then
			self.Owner:EmitSound(Confirm)
			self.Wep:SetTarget(braintrace.Entity)
			self.Owner:ChatPrintFormatted("#gstands.lovers.target", braintrace.Entity:GetName())
...
function ENT:DrawTranslucent()
	...
	if !(LocalPlayer():GetActiveWeapon() and LocalPlayer():IsValid() and IsPlayerStandUser(LocalPlayer())) and IsValid(self:GetTrail()) then
		...
		elseif (LocalPlayer():GetActiveWeapon() and LocalPlayer():IsValid() and IsPlayerStandUser(LocalPlayer())) and IsValid(self:GetTrail()) and LocalPlayer() != self.Owner then
		...
		if CLIENT and LocalPlayer():GetActiveWeapon() then
			if LocalPlayer():IsValid() and IsPlayerStandUser(LocalPlayer()) then
				...
				if !Active and !self.OwnerNoDraw and self.LOD < 2 then
					...
					if self.LOD < 2 then
						self.StandAura = self.StandAura or CreateParticleSystem(self, "loversaura", PATTACH_POINT, 1)
